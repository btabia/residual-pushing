...
class ArmSim(BasePybulletEnv):
    ...
    def get_jacobian(self, joint_positions, joint_velocities, tool_centre):
        print("joint_positions: " + str(joint_positions))
        joint_acceleration = [0,0,0,0,0,0,0]
        ee_index = self.robot_end_effector_link_index
        tool_centre = np.array([0,0,0])
        num_joint = self._p.getNumJoints(self.robot_arm_id)
        print("num_joint: " + str(num_joint))
        jacobian  = self._p.calculateJacobian(self.robot_arm_id, 
                                              ...
                                              tool_centre, #tool_centre,
                                               joint_acceleration,#joint_positions.tolist(),
                                               joint_acceleration,#joint_velocities.tolist(),
                                              joint_acceleration
                                              )
    ...
    def update_joint_states(self):
        ...
        print("self._robot_joint_indices: " + str(self._robot_joint_indices))
