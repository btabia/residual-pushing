...
class Pushing(ArmSim):
    ...
    def _set_observation_space(self):
        ...
        # ROBOT joint position state : 6
        # object orientation : 4 (quaternion (wxyz))
        # object size : length - width: 2
        # Total : 19
    ...
    def _get_observation(self):
        ...
        # object world state
        self.obj_lin_pos, self.obj_orientation = self._p.getBasePositionAndOrientation(self.pushing_object_id)#, self.client_id)
        ...
        #print("self.debris_target_vel: " + str(self.debris_target_vel))
        ...
        data = {
            ...
            "ee_ang_velocities": self.ee_ang_velocities.tolist(),
            "tool_object_pos": self.tool_object_pos.tolist(),
    ...
    def reset_task(self):
        ...
        x_ee_pos = central_target[0] + (obj_distance_min + np.random.rand() * delta_dist) * np.cos(ee_ang)
        y_ee_pos = central_target[1] + (obj_distance_min + np.random.rand() * delta_dist) * np.sin(ee_ang)
        z_ee_pos = np.random.rand() * 0.5
        ee_pos = np.array([x_ee_pos, y_ee_pos, z_ee_pos])
        x_target_pos = central_target[0] + (obj_distance_min + np.random.rand() * delta_dist) * np.cos(ee_ang)
        y_target_pos = central_target[1] + (obj_distance_min + np.random.rand() * delta_dist) * np.sin(ee_ang)
        target_pos = central_target#np.array([x_target_pos, y_target_pos, 0])
        ...
        self.goal_obj_conf = self.utils.generate_fixed_goal(target_pos)
    ...
    def get_reward(self):
        ...
        reward_displacement_tolerance = self.cfg["RL"]["reward"]["object_goal_speed_tolerance"]
