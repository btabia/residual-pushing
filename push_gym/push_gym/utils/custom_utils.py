...
#from utils.planner.new_bi_rrt import birrt
#from utils.planner.meta import direct_path
...
def remove_body(body, _p):
    return _p.removeBody(body)


def get_pose(body, _p, p_client):
    return _p.getBasePositionAndOrientation(body, physicsClientId=p_client)
    # return np.concatenate([point, quat])

def get_config(o_id, _p, p_client):
    return [list(get_point(o_id, _p, p_client)), list(get_quat(o_id, _p, p_client))]

def get_point(body, _p, p_client):
    return get_pose(body, _p, p_client)[0]


def get_quat(body, _p, p_client):
    return get_pose(body, _p, p_client)[1]  # [x,y,z,w]


def get_euler(body, _p):
    return euler_from_quat(get_quat(body, _p, p_client))


def get_base_values(body):
    return base_values_from_pose(get_pose(body))

def sample_pos_in_env(bounds, _p, z=None, dist=None):
    if dist != None: 
        alpha = np.random.rand() * np.pi
        xpos = np.random.rand() * dist * np.cos(alpha)
        ypos = np.random.rand() * dist * np.sin(alpha)
    else: 
        xpos = random.uniform(bounds[0, 0], bounds[0, 1])
        ypos = random.uniform(bounds[1, 1], bounds[1, 0])
    zpos = bounds[2, 0]
    if z:
        zpos = z
    yaw = _p.getQuaternionFromEuler([0, 0, random.uniform(-np.pi, np.pi)])
    return xpos,ypos, zpos, yaw

def set_pose(body, pose, _p, p_client):
    (point, quat) = pose
    p.resetBasePositionAndOrientation(body, point, quat, physicsClientId=p_client)


def set_point(body, point, _p, p_client):
    set_pose(body, (point, get_quat(body, _p, p_client)), _p, p_client)


def set_quat(body, quat, _p, p_client):
    set_pose(body, (get_point(body), quat), _p, p_client)


def set_euler(body, euler, _p):
    set_quat(body, quat_from_euler(euler), _p)
...
def get_box_geometry(width, length, height, _p):
    return {
        'shapeType': _p.GEOM_BOX,
...
def create_collision_shape(geometry, _p, p_client, pose=unit_pose()):
    point, quat = pose
    collision_args = {
    ...
    return _p.createCollisionShape(**collision_args)


def create_visual_shape(geometry, _p, p_client, pose=unit_pose(), color=RED, specular=None):
    ...
    return _p.createVisualShape(**visual_args)


def create_shape(geometry, _p, p_client, pose=unit_pose(), collision=True, **kwargs):
    collision_id = create_collision_shape(geometry, _p, p_client, pose=pose) if collision else NULL_ID
    visual_id = create_visual_shape(geometry, _p, p_client, pose=pose, **kwargs)  # if collision else NULL_ID
...
def create_body(_p,p_client, collision_id=NULL_ID, visual_id=NULL_ID, mass=STATIC_MASS):
    return _p.createMultiBody(baseMass=mass, baseCollisionShapeIndex=collision_id,
                             baseVisualShapeIndex=visual_id, physicsClientId=p_client)
...
def create_box(_p,p_client, w, l, h, mass=STATIC_MASS, color=RED, **kwargs):
    collision_id, visual_id = create_shape(get_box_geometry(w, l, h, _p), _p, p_client, color=color, **kwargs)
    return create_body(_p,p_client, collision_id, visual_id, mass=mass)
    # basePosition | baseOrientation
    # linkCollisionShapeIndices | linkVisualShapeIndices


def create_cylinder(_p,p_client,radius, height, mass=STATIC_MASS, color=BLUE, **kwargs):
    collision_id, visual_id = create_shape(get_cylinder_geometry(radius, height),_p,p_client, color=color, **kwargs)
    return create_body(_p, p_client, collision_id, visual_id, mass=mass)


def create_capsule(_p,p_client,radius, height, mass=STATIC_MASS, color=BLUE, **kwargs):
    collision_id, visual_id = create_shape(get_capsule_geometry(radius, height),_p,p_client, color=color, **kwargs)
    return create_body(_p,p_client,collision_id, visual_id, mass=mass)


def create_sphere(_p,p_client,radius, mass=STATIC_MASS, color=BLUE, **kwargs):
    collision_id, visual_id = create_shape(get_sphere_geometry(radius),_p,p_client, color=color, **kwargs)
    return create_body(_p,p_client,collision_id, visual_id, mass=mass)
