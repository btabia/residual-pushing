...
def test_agent(tf_model_path, evaluation_save_path, log_dir_name, obstacle_num, env_name, arm_position, test_baseline,
    ...
    env.setup_for_RL(tf_model, obstacle_num, arm_position, False, evaluation_save_path, False)
    eval_max_range = 1000
    ...
    if test_baseline:
        for i in tqdm(range(eval_max_range)):
            if evaluation_samples:
                env.evaluation_sample = evaluation_samples['samples'][i]
            _ = env.reset()
            _ = env.baseline.create_corridors()

    else:
        saved_model_path = log_dir_name + "/best_model"
